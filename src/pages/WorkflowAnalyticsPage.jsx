...
const METRIC_EXPLANATIONS = {
    ...
    logistica_table: {
        title: "Mapeamento da Visão Logística",
        formula: (
            <ul className="list-disc pl-4 space-y-1 mt-1 text-[11px]">
                <li><strong>Nº PI:</strong> Nº do Processo de Importação (Identificação).</li>
                <li><strong>Nº Factura:</strong> Nº da Fatura Comercial.</li>
                ...
                <li><strong>Entrega(RCS):</strong> Data de entrega física ou RCS.</li>
                <li><strong>Factura → Despacho:</strong> Dias corridos calculados entre a Data da Factura e o Despacho.</li>
                <li><strong>Chegada(AO) → Entrega(RCS):</strong> Dias corridos calculados entre a Chegada AO e a entrega física.</li>
                <li><strong>Factura → Entrega(RCS):</strong> Lead Time total de ponta a ponta.</li>
                <li><strong>Comentário:</strong> Observações gerais do processo.</li>
            </ul>
        ),
        source: "Mapeamento direto dos metadados e histórico dos workflows do DocuWare.",
    ...
    consolidada_table: {
        title: "Mapeamento da Visão Consolidada",
        formula: (
            <ul className="list-disc pl-4 space-y-1 mt-1 text-[11px]">
                <li><strong>Nº PI:</strong> Nº do Processo de Importação (Identificação).</li>
                <li><strong>Nº Factura:</strong> Nº da Fatura Comercial.</li>
...
const WorkflowAnalyticsPage = ({ activeTab: controlledActiveTab, onTabChange }) => {
    ...
    const exportPurchasingToCSV = () => {
        const headers = [
            'Nº PI', 'Empresa', 'Nº Factura', 'Data Factura', 'Tipo', 'Chegada AO', 'Entrada (RCS)', 
    ...
    const exportLogisticaToCSV = () => {
        const headers = [
            'Nº PI', 'Empresa', 'Nº Factura', 'Tipo', 'Transportador', 'Transitário', 'Data da Factura', 'Data de Despacho', 'Chegada AO', 'Entrega(RCS)', 'Factura → Despacho', 'Chegada(AO) → Entrega(RCS)', 'Factura → Entrega(RCS)'
        ];
    ...
    const exportConsolidadaToCSV = () => {
        const headers = [
            'Nº PI', 'Empresa', 'Nº Factura', 'Tipo', 'Transportador', 'Transitário',
        ...
        return (
            <th className={`bg-[#d0ebf8] sticky top-0 z-10 p-2 border-b border-slate-200 ${widthClass} ${align === 'center' ? 'text-center' : align === 'right' ? 'text-right' : ''}`}>
                <div className={`inline-flex items-center gap-1.5 ${align === 'center' ? 'justify-center' : align === 'right' ? 'justify-end' : 'justify-between'}`}>
                    ...
                    <div className={`dropdown dropdown-bottom ${colKey === 'docNum' || colKey === 'etapa' || colKey === 'fornecedor' ? '' : 'dropdown-end'} inline-block`}>
    ...
    const detailedProcesses = useMemo(() => {
        return filteredDocuments.map(doc => {
            ...
            return {
                ...
                montanteTransporte: fFrete,
                despesasExtras: fCustosAdicionais,
                frete: fFreteKz, 
                freteOrig: fFrete,
                freteFC: fFreteFCKz,
                ...
                coefImportacao: (() => {
                    const facturaKz = parseFloat(fMercFCKz);
                    if (!facturaKz || facturaKz === 0) return '-';
                    const transporte = parseFloat(fFrete) || 0;
                    const despesasExtras = parseFloat(fCustosAdicionais) || 0;
                    const cambioFC = parseFloat(fValorCambialFC) || 1;
                    const direitosFC = parseFloat(fDireitosFC) || 0;
                    const servicosDespachanteFC = parseFloat(fServicosDespachanteFC) || 0;
                ...
                coefImportacaoNumeric: (() => {
                    const facturaKz = parseFloat(fMercFCKz);
                    if (!facturaKz || facturaKz === 0) return null;
                    const transporte = parseFloat(fFrete) || 0;
                    const despesasExtras = parseFloat(fCustosAdicionais) || 0;
                    const cambioFC = parseFloat(fValorCambialFC) || 1;
                    const direitosFC = parseFloat(fDireitosFC) || 0;
                    const servicosDespachanteFC = parseFloat(fServicosDespachanteFC) || 0;
                    const numerator = (transporte * cambioFC) + (despesasExtras * cambioFC) + direitosFC + servicosDespachanteFC;
                    return (numerator / facturaKz) * 100;
                })(),
                diasTotais,
                isParcial,
                diasParado,
    ...
    const searchedAndSortedDetails = useMemo(() => {
        ...
        if (detailSort.column) {
            result.sort((a, b) => {
                ...
                else if (col === 'coefImportacao') col = 'coefImportacaoNumeric';
                                    ...
                                    <table className="table table-compact w-full text-[11px] border-collapse">
                                        <thead>
                                            <tr className="bg-slate-50 text-slate-600 font-bold">
                                                ...
                                                {renderFilterHeader('Empresa', 'fornecedor', 'w-[150px] min-w-[150px] max-w-[150px]')}
                                                ...
                                                {renderFilterHeader('Tipo Carga', 'tipoCarga', 'w-[120px] min-w-[120px] max-w-[120px]')}
                                            ...
                                            {filteredDetailsForPurchasing.map((p) => {
                                                ...
                                                return (
                                                    <tr key={p.id} className="hover:bg-slate-50 border-b border-slate-100">
                                                        ...
                                                        <td className="text-slate-600 whitespace-nowrap w-[150px] min-w-[150px] max-w-[150px] truncate" title={p.fornecedor}>{p.fornecedor}</td>
                                                        ...
                                                        <td className="whitespace-nowrap truncate w-[120px] min-w-[120px] max-w-[120px]" title={p.tipoCarga}>{p.tipoCarga}</td>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                                ...
                                <div className="text-xs text-slate-500 font-bold flex items-center gap-2">
                                    ...
                                    <button 
                                        onClick={() => setVisibleChartExplanations(prev => ({...prev, logistica_table: !prev.logistica_table}))}
                                        className="btn btn-xs btn-ghost text-slate-400 hover:text-indigo-600 p-0 hover:bg-transparent"
                                        title="Ver Detalhes do Mapeamento de Campos"
                                    >
                                        <FaInfoCircle className="text-sm" />
                            ...
                            <ChartInfoAlert 
                                metricKey="logistica_table" 
                                showInfo={!!visibleChartExplanations['logistica_table']} 
                                setShowInfo={(val) => setVisibleChartExplanations(prev => ({...prev, logistica_table: val}))} 
                            />
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex flex-col md:flex-row gap-4 items-center justify-between mt-6">
                                ...
                                <div className="text-xs text-slate-500 font-bold flex items-center gap-2">
                                    ...
                                    <button 
                                        onClick={() => setVisibleChartExplanations(prev => ({...prev, consolidada_table: !prev.consolidada_table}))}
                                        className="btn btn-xs btn-ghost text-slate-400 hover:text-indigo-600 p-0 hover:bg-transparent"
                                        title="Ver Detalhes do Mapeamento de Campos"
                                    >
                                        <FaInfoCircle className="text-sm" />
                            ...
                            <ChartInfoAlert 
                                metricKey="consolidada_table" 
                                showInfo={!!visibleChartExplanations['consolidada_table']} 
                                setShowInfo={(val) => setVisibleChartExplanations(prev => ({...prev, consolidada_table: val}))} 
                            />
